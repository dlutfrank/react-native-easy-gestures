...
export default class Gestures extends Component {
  static propTypes = {
    ...
    singleRotatable: PropTypes.bool,
  ...
  onDrag(event, gestureState) {
    if (!isMultiTouch(event)) {
      const { singleRotatable } = this.props;
      if (singleRotatable) {
        const { style } = this.state;
        ...
        const currentScale = transform
          .map((s) => s.scale)
          .reduce((a, b) => b || a, 1);
        this.pinchStyles = { transform: [{ scale: currentScale }] };
  ...
  onRotate = (event) => {
    ...
    const { initialTouches, anchor } = this;

    if (rotatable || singleRotatable) {
      const events = getTouches(event);
      const isSingle = singleRotatable && events.length < 2;
      const currentAngle = isSingle ? angle(events, anchor) : angle(events);
      let initialAngle =
        initialTouches.length > 1 ? angle(initialTouches) : currentAngle;
      if (isSingle) {
        initialAngle = angle(initialTouches, anchor);
      }
  ...
  handleLayout = (event) => {
    ...
    this.center = { x: Math.round(x + width / 2), y: Math.round(y + height / 2) };
  };

  updateAnchor = (event) => {
    const { x, y } = getOffset(event);
    const { x: cx, y: cy } = this.center;
    this.anchor = { x: cx + x, y: cy + y };
  };
